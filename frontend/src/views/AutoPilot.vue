    ...
    <div class="page-body ap-layout">
      <div class="ap-left">
        ...
        <div class="card">
          ...
          <div class="stat-grid">
            ...
            <div class="stat-item"><div class="stat-num">{{ dailyLimitDisplay }}</div><div class="stat-lbl">今日上限</div></div>
            <div class="stat-item"><div class="stat-num">{{ remainingLimitDisplay }}</div><div class="stat-lbl">剩余额度</div></div>
      ...
      <div class="ap-right">
        <div class="card">
          ...
          <div class="cfg-section">
            <div class="cfg-section-title">每日图片下载上限</div>
            <div class="cfg-grid">
              <div class="form-row">
                <label>上限策略</label>
              ...
              <div class="form-row">
                <label>手动上限</label>
                <input v-if="cfg.daily_limit_mode === 'manual'" class="input" type="number" min="1" max="500" v-model.number="cfg.manual_daily_limit" placeholder="1-500" @change="onCfgChange" />
                <div v-else class="inline-note inline-note--muted">自动模式下不可编辑，当前自动值 {{ dailyLimitDisplay }} 张</div>
              </div>
            </div>
          ...
          <div class="cfg-section">
            ...
            <div class="cfg-grid">
              ...
              <div class="form-row"><label>排序方式</label><select class="select" v-model="cfg.sort_by" @change="onCfgChange"><option value="yesterday_hot">昨日热门</option><option value="3days_hot">近三天热门</option><option value="7days_hot">上周热门</option><option value="latest">最新</option><option value="most_views">推荐的</option></select></div>
...
const createDefaultAutoPilotConfig=()=>({timezone:'Asia/Shanghai',active_start:8,active_end:23,daily_limit_mode:'auto',manual_daily_limit:null,active_session_min:5,active_session_max:20,active_interval_min:1800,active_interval_max:7200,inactive_enabled:false,inactive_session_min:2,inactive_session_max:8,inactive_interval_min:7200,inactive_interval_max:14400,use_imgbed_upload:false,wallpaper_type:'static',sort_by:'yesterday_hot',categories:[],color_themes:[],vip_only:false,min_hot_score:0,tag_blacklist:[],min_width:null,min_height:null,screen_orientation:'all'})
...
const normalizeAutoPilotConfig=remote=>{const merged={...createDefaultAutoPilotConfig(),...(remote||{})};merged.daily_limit_mode=String(merged.daily_limit_mode||'auto').toLowerCase()==='manual'?'manual':'auto';merged.manual_daily_limit=merged.manual_daily_limit?Math.min(500,Math.max(1,Number(merged.manual_daily_limit)||0)):null;[merged.active_session_min,merged.active_session_max]=normalizeRange(merged.active_session_min,merged.active_session_max,5,20,1);[merged.inactive_session_min,merged.inactive_session_max]=normalizeRange(merged.inactive_session_min,merged.inactive_session_max,2,8,1);[merged.active_interval_min,merged.active_interval_max]=normalizeRange(merged.active_interval_min,merged.active_interval_max,1800,7200,60);[merged.inactive_interval_min,merged.inactive_interval_max]=normalizeRange(merged.inactive_interval_min,merged.inactive_interval_max,7200,14400,60);merged.active_start=clampNumber(merged.active_start,8,0)%24;merged.active_end=clampNumber(merged.active_end,23,0)%24;merged.min_hot_score=Math.max(0,Number(merged.min_hot_score)||0);merged.min_width=merged.min_width?Math.max(1,Number(merged.min_width)||0):null;merged.min_height=merged.min_height?Math.max(1,Number(merged.min_height)||0):null;return merged}
...
const dailyLimitHint=computed(()=>{const effectiveLimit=status.value.today?.daily_limit;const remaining=status.value.today?.remaining;if(!backendLimitReady.value)return'当前后端还没有返回今日上限数据，通常是服务仍在运行旧版本代码。重启后端后，这里会显示自动值和剩余额度。';if(cfg.value.daily_limit_mode==='manual'){const manualLimit=cfg.value.manual_daily_limit||effectiveLimit||'--';return`手动模式会立即覆盖今日上限，当前生效 ${manualLimit} 张，剩余 ${remaining ?? '--'} 张。`}return`自动模式会按人性化策略每天生成不同上限，当前自动值 ${effectiveLimit ?? '--'} 张，剩余 ${remaining ?? '--'} 张。`})
...
.inline-note--muted{color:var(--text-3);background:var(--bg-hover)}
