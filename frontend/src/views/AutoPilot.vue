        ...
        <div class="card">
          ...
          <div class="cfg-section">
            ...
            <div class="automation-grid">
              <div class="sub-card"><div class="sub-card__title">动态图转换</div><label class="toggle-row toggle-row--compact"><span class="toggle-row-label">启用动态图自动转换</span><label class="toggle"><input type="checkbox" v-model="mediaSettings.video.enabled" @change="onCfgChange" /><span class="toggle-track"></span></label></label><div class="field-grid"><div class="form-row"><label>输出格式</label><select class="select" v-model="mediaSettings.video.output_format" @change="onCfgChange"><option value="webp">动态 WebP</option><option value="gif">GIF</option></select></div><div class="form-row"><label>帧率</label><input class="input" type="number" min="1" max="60" v-model.number="mediaSettings.video.fps" @change="onCfgChange" /></div><div class="form-row"><label>最大帧数</label><input class="input" type="number" min="10" max="600" v-model.number="mediaSettings.video.max_frames" @change="onCfgChange" /></div><div class="form-row"><label>最大宽度</label><input class="input" type="number" min="0" v-model.number="mediaSettings.video.max_width" @change="onCfgChange" /></div><div class="form-row"><label>质量</label><input class="input" type="number" min="1" max="100" v-model.number="mediaSettings.video.quality" @change="onCfgChange" /></div><div class="form-row form-row--full"><label class="toggle-row toggle-row--compact"><span class="toggle-row-label">转换后删除原始视频</span><label class="toggle"><input type="checkbox" v-model="mediaSettings.video.delete_original" @change="onCfgChange" /><span class="toggle-track"></span></label></label></div></div></div>
...
const createDefaultMediaSettings=()=>({auto_convert:false,max_concurrent:1,video:{enabled:false,output_format:'webp',fps:10,max_frames:120,width:0,max_width:1280,quality:80,delete_original:false,timeout_seconds:300,cpu_nice:5},image:{enabled:false,output_format:'webp',quality:85,delete_original:false,timeout_seconds:120,cpu_nice:5}})
...
const buildMediaSettingsPayload=()=>{const normalized=normalizeMediaSettings(mediaSettings.value);return{auto_convert:!!normalized.auto_convert,max_concurrent:normalized.max_concurrent||1,video:{...normalized.video},image:{...normalized.image}}}
