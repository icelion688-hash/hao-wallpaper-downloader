...
class Downloader:
    ...
    async def download(
        ...
        if os.path.exists(local_path) and os.path.getsize(local_path) > 0:
            ...
            return DownloadResult(success=True, local_path=rel)
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                ...
                last_result = result
                logger.warning(
                    "[Downloader] %s 第 %d 次下载失败: %s",
                    resource_id,
                    attempt,
                    result.failure_reason or "未知原因",
                )

            except Exception as e:
                last_result = DownloadResult(
                    success=False,
                    failure_reason=str(e) or "未知异常",
                    failure_stage="download",
                    retryable=True,
                )
        ...
        logger.error(
            "[Downloader] %s 所有重试均失败: %s",
            resource_id,
            last_result.failure_reason or "未知原因",
        )
        return last_result
        ...
        async with self.anti.build_client(cookie, profile=session_profile) as client:
            ...
            try:
                async with client.stream(
                    ...
                    if resp.status_code == 200:
                        ...
                        return DownloadResult(
                            ...
                            retryable=resp.status_code >= 500 or resp.status_code in {408, 409, 425, 429},
                    ...
                    if total > 0 and downloaded < total * 0.99:
                        ...
                        return DownloadResult(
                            success=False,
                            failure_reason=f"文件不完整 ({downloaded}/{total} bytes)",
                ...
                return DownloadResult(
                    success=False,
                    failure_reason=(
                        f"下载超时 (已下载 {_fmt_bytes(downloaded)} / {_fmt_bytes(total)})"
                    ),
                ...
                logger.error("[Downloader] %s 网络异常: %s", resource_id, e)
                return DownloadResult(
                    success=False,
                    failure_reason=str(e) or "网络异常",
                    failure_stage="network",
                    retryable=True,
                )
            except OSError as e:
                logger.error("[Downloader] %s 本地文件写入异常: %s", resource_id, e)
                return DownloadResult(
                    success=False,
                    failure_reason=str(e) or "本地文件写入异常",
